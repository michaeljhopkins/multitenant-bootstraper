...
class ReverseSeedCommand extends Command {
	...
	public function fire()
	{
		...
		foreach ($tables as $table) {
			...
			if(!\File::exists($fileName) || $this->option('force')) {
				$this->printResult(\ReverseSeed::generateSeed($table, $this->option('database'), $chunkSize), $table);
				continue;
			}
    ...
    protected function generateFileName($table)
    {
		...
		$className = \ReverseSeed::generateClassName($table);
		$seedPath = base_path(\Config::get('reverseSeed.path'));
